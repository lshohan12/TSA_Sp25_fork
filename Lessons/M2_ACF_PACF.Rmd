...
#ggplot(inflow_data, aes(x=my_date))  #not finished
...
ts_inflow_data <- ts(inflow_data$HP1, start = c(1931, 1), frequency = 12)
  #frequency of 12 because of months
ts_inflow_data4 <- ts(inflow_data[,2:4], start = c(1931, 1), frequency = 12)
ts_inflow_data
...
```{r}
#autoplot is ggplot for time series data

autoplot(ts_inflow_data4) +
  xlab("Time") +
  ylab("Inflow") +
...
#lags are months - with capitalized acf and pacf it looks better!!!

HP1_acf=acf(ts_inflow_data4[,1], lag = 60) 
  #lag 0 is from month 1 to month 1,
  # 2 bar shows that correlation at lag 1 is between 0.8 so strong correlation of what happened in month before onto the next month
  #correlation slowly goes down over time, relationship decreases so months arent as correlated over 3-4 month span
  #strong seasonal component reflected in ACF 
HP1_acf=Acf(ts_inflow_data4[,1], lag = 60) 

#Option 2: Computing ACF and PACF with package "forecast"
HP1_pacf=pacf(ts_inflow_data4[,1], lag = 60)
